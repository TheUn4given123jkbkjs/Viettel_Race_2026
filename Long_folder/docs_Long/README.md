...
| :---: | :---: | :--- | :--- |
| **v6.1** | 2026-07-30 | `run_v3_groq_isolated.bat`<br>`groq_runner.py`<br>`generate_train_data_v3.py`<br>`key_manager.py`<br>`models_registry.json` | 1. **Triển khai Kiến trúc Model Isolation (`run_v3_groq_isolated.bat`):** Phân lập luồng chạy chuyên trách theo từng Model, hỗ trợ tùy chỉnh số lượng Workers cho 70B, 8B và Qwen 27B trực tiếp trên CMD.<br>2. **Tái cấu trúc Luồng Tương tác (`goto` labels):** Loại bỏ khối lệnh `if (...)` lồng nhau của CMD, khắc phục 100% lỗi tự động đóng cửa sổ CMD.<br>3. **Sửa Triệt Để Lỗi HTTP 400 (Groq Proxy WAF):** Bỏ hoàn toàn tham số `response_format: json_object` khi gửi lên Groq Cloud để khỏi bị Groq chặn HTTP 400 (`Failed to generate JSON`), đảm bảo 100% phản hồi `STATUS 200 OK` siêu tốc. Chuyển sang trích xuất tự động bằng Python Regex `re.search(r'\{.*\}', content_text, re.DOTALL)` cắt sạch thẻ `<think>` và markdown ```json lấy chuẩn xác 100% đối tượng JSON trên cả 3 models.<br>4. **Chuẩn hóa Báo cáo `api_report.md`:** Cập nhật `KeyInfo.mark_rate_limited` luôn cộng 1 vào cột `Thất Bại` khi bị ngắt luồng 429/pause.<br>5. **Tối ưu Pacing 70B (12.0s):** Nâng dãn cách Llama 70B lên 12.0s/req để khớp chuẩn trần 12,000 TPM của Groq. |
