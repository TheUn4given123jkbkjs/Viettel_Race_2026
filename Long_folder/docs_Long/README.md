...
| :---: | :---: | :--- | :--- |
| **v6.1** | 2026-07-30 | `run_v3_groq_isolated.bat`<br>`groq_runner.py`<br>`generate_train_data_v3.py`<br>`key_manager.py`<br>`models_registry.json` | 1. **Triển khai Kiến trúc Model Isolation (`run_v3_groq_isolated.bat`):** Phân lập luồng chạy chuyên trách theo từng Model, hỗ trợ tùy chỉnh số lượng Workers cho 70B, 8B và Qwen 27B trực tiếp trên CMD.<br>2. **Tái cấu trúc Luồng Tương tác (`goto` labels):** Loại bỏ khối lệnh `if (...)` lồng nhau của CMD, khắc phục 100% lỗi tự động đóng cửa sổ CMD.<br>3. **Khai báo `max_tokens: 4096` Triệt tiêu Lỗi JSON Truncation:** Bổ sung `"max_tokens": 4096` vào payload Groq Cloud, chống lại việc Groq mặc định ngắt đuôi JSON ở token 1024 (triệt tiêu 100% lỗi `JSONDecodeError`).<br>4. **Tối ưu Pacing Delay 8B (8.0s):** Nâng `rpm_delay_seconds` của 8B từ 2.0s lên **8.0s** để giữ lưu lượng 2,500 tokens/mẫu luôn an toàn dưới trần **6,000 TPM** của Groq Free Tier.<br>5. **Chuẩn hóa Báo cáo `api_report.md`:** Cập nhật `KeyInfo.mark_rate_limited` luôn cộng 1 vào cột `Thất Bại` khi bị ngắt luồng 429/pause. |
